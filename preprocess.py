    ...
    while sentinels < n_workers:
        ...
        if wav is None:
            ...
            try:
                ...
                with torch.no_grad():
                    # Pass native file SR; each extractor resamples internally.
                    # MimiExtractor  → resamples to 24,000 Hz
                    # HuBERTExtractor → resamples to 16,000 Hz (Ditto pipeline)
                    tokens = mimi_ext.extract(wav, file_sr)    # (T_m, num_codebooks) at 12.5 Hz
                    hubert = hubert_ext.extract(wav, file_sr)  # (T_h, feat_dim) at 25 Hz
                    # NO upsampling — Ditto HuBERT ONNX is already 25 Hz;
                    # bridge upsample_factor=2 also gives 25 Hz. Rates match.
